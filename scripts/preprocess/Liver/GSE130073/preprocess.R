...
file_path <- "GSM3731527/features.tsv.gz"
...
for (donor in gsm_ids){
  ...
  # Read metadata
  metadata <- read.csv('Epithelial.csv',header = T,row.names = 1) # Self-extracted epithelial barcodes
  donorObj <- AddMetaData(donorObj,metadata = metadata)
  donorObj <- subset(donorObj,subset=Epithelial=='Epithelial')
