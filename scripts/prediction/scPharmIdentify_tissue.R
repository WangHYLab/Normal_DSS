...
#' @param tissue the tissue of cell. This can be one of:
#'   - "Bladder"
#'   - "Blood"
#'   - "Bowel"
#'   - "Brain"
...
#'   - "Uterus"
#'   - "pan"
#'   tissue='pan' means calculating in the context of pan-tissue
...
#' }
scPharmIdentify_tissue <- function(object,
                            type,
                            cancer,
                            drug = NULL,
                            tissue = NULL,
  ...
  if (is.character(tissue) && length(tissue) == 1) {
    if (tissue != "pan") {
      if (tissue %in% unique(GDSC$Tissue)) {
        GDSC <- GDSC[GDSC$Tissue == tissue, ]
      } else {
        message("ERROR: unrecognized tissue type")
      }
    }
  } else if (is.vector(tissue) && length(tissue) > 1) {
    if (length(intersect(tissue, unique(GDSC$Tissue))) == length(tissue)) {
      GDSC <- GDSC[which(GDSC$Tissue %in% tissue), ]
    } else {
      message("ERROR: contains the wrong tissue type")
    }
  } else {
    message("ERROR: unrecognized parameter")
  }
