...
datasets <- data.frame(tissue = c('Bladder','Stomach'),
                 project_names = c('GSE134355','GSE134520'))

for (row in 1:nrow(datasets)){
  
  tissue <- datasets[row,1]
  project_name <- datasets[row,2]
  DATA_DIR <- paste('/home/chenshuting/drug_toxicity/pan_tissue', tissue, project_name, 'raw_data/', sep = '/')
...
print(names(seurat_objects))
saveRDS(seurat_objects,paste0(WORK_DIR,'Rdata/seurat_objects_bf_sample_imm.rds')) # Selected datasets before sampling

seurat_objects_bf <- seurat_objects


seurat_objects <- readRDS(paste0(WORK_DIR,'Rdata/seurat_objects_bf_sample_imm.rds'))


cell_counts <- cell_counts %>%
  mutate(Sample_num_cells = ifelse(as.numeric(Num_Cells) < 300, as.numeric(Num_Cells), 300))
cell_counts$Sample_num_cells[cell_counts$Project == 'PRJNA662018' & 
                               cell_counts$Tissue == 'Bladder'] <- 404
cell_counts$Sample_num_cells[cell_counts$Project == 'GSE159929' & 
                               cell_counts$Tissue == 'Bladder'] <- 403
cell_counts$Sample_num_cells[cell_counts$Project == 'GSE159929' & 
                               cell_counts$Tissue == 'Stomach'] <- 631

write.csv(cell_counts,
          paste0(WORK_DIR,'Tables/sample_disco_cells_for_imm_null_distribution.csv'),
          row.names = F)
...
for (key in names(seurat_objects)) {
  ...
  if (ncol(seurat_obj) > target_cells) {
    ...
    sampled_cells <- sample(colnames(seurat_obj), target_cells, replace = TRUE) # Use replacement sampling if insufficient cells
    seurat_objects[[key]] <- subset(seurat_obj, cells = sampled_cells)
    message(paste("Sampling", key, "with replacement as it has", ncol(seurat_obj), "cells"))
  }
}
...
saveRDS(seurat_objects,
        paste0(WORK_DIR,'Rdata/seurat_objects_for_imm_null_distribution.rds'))
